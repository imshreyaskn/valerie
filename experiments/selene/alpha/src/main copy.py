...
class ResponseSchema(BaseModel):
    ...
    godmode_response: GodmodeResponse
...
class SeleneAgent:
    ...
    def _initialize_llm(self, api_key: str, model: str) -> ChatMistralAI:
    ...
    def _create_prompt_template(self) -> ChatPromptTemplate:
        ...
        system_prompt = SystemMessagePromptTemplate.from_template(self._get_system_prompt())
        human_prompt = HumanMessagePromptTemplate.from_template(self._get_human_prompt())
        
        return ChatPromptTemplate.from_messages([system_prompt, human_prompt])

    def _get_system_prompt(self) -> str:
        """Return the system prompt for red teaming."""
        return SYSTEM_PROMPT
    
    def _get_human_prompt(self) -> str:
        return HUMAN_PROMPT

    def process_query(self, user_query: str) -> Optional[RootSchema]:
        """Process a user query and return the structured response."""
        try:
            # Validate input
            ...
            # Format messages
            messages = self.chat_prompt.format_messages(user_query=user_query.strip())
    ...
    def print_response_summary(self, response: RootSchema) -> None:
        ...
        if hasattr(response, 'response') and hasattr(response.response, 'godmode_response'):
            print("GODMODE RESPONSE:")
            print("-" * 30)
            print(response.response.godmode_response.prompt)
        
        if hasattr(response, 'response') and hasattr(response.response, 'ethical_disclaimer'):
            print("\nETHICAL DISCLAIMER:")
            print("-" * 30)
            print(response.response.ethical_disclaimer.message)
