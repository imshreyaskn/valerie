...
export function Inspector() {
  ...
  const selectedTask = usePipelineStore((s) => (selectedTaskId ? s.liveTasks[selectedTaskId] || null : null));
    ...
    <AnimatePresence>
      {inspectorOpen && (
        ...
        >
          {/* Resize handle */}
          <div
            className="absolute left-0 top-0 bottom-0 w-1.5 cursor-col-resize hover:bg-slate/30 active:bg-slate/50 z-10 transition-colors"
          ...
          <div className="flex items-center justify-between px-4 py-3 bg-linen/40 border-b border-hairline shrink-0 select-none">
            ...
            <button
              ...
              className="p-1.5 rounded-xs text-steel hover:text-slate hover:bg-linen transition-colors cursor-pointer"
