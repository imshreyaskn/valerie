import { useEffect, useState, useMemo, useCallback } from 'react';
import { usePipelineStore } from '../stores/pipelineStore';
import { useWorkspaceStore } from '../stores/workspaceStore';
import { useRunStream } from '../hooks/useRunStream';
import { CommandBar, InstrumentCluster, ExecutionCircuit } from '../components/mission-control/CommandBar';
import type { FilterState } from '../types/filters';
import { DEFAULT_FILTERS, matchesFilters, hasActiveFilters as filtersActive } from '../types/filters';
import { SpecimenLedger } from '../components/mission-control/SpecimenLedger';
import { IntelligenceFeed } from '../components/IntelligenceFeed';
import { useLauncherStore } from '../stores/launcherStore';
import { api } from '../utils/api';
import { HERO_ASCII_ART } from '../constants/ascii';
import { AlertTriangle, X, RefreshCw } from 'lucide-react';

export default function MissionControl() {
  const { setActiveRun, setActiveRunMeta, activeRunId, activeRunMeta } = usePipelineStore();
  const openInspector  = useWorkspaceStore(s => s.openInspector);
  const openPromptDiff = useWorkspaceStore(s => s.openPromptDiff);
  const selectedEntity = useWorkspaceStore(s => s.selectedEntity);
  const openLauncher   = useLauncherStore(s => s.openLauncher);

  useRunStream(activeRunId || 'all');

  useEffect(() => {
    if (!activeRunId) setActiveRun('all');
  }, [activeRunId, setActiveRun]);

  // Hydrate run results whenever activeRunId changes or on initial mount
  useEffect(() => {
    let cancelled = false;
    const hydrate = async () => {
      const targetRunId = activeRunId && activeRunId !== 'all' ? activeRunId : null;
      if (targetRunId) {
        try {
          const runDoc = await api.getRun(targetRunId);
          if (!cancelled && runDoc) {
            usePipelineStore.getState().setActiveRunMeta({
              domain: runDoc.domain,
              endpoint_id: runDoc.endpoint_id,
              endpoint_name: runDoc.endpoint_id,
              attacker_model: runDoc.attacker_model,
              judge_model: runDoc.judge_model,
              started_at: runDoc.created_at,
              status: runDoc.status,
              error_message: runDoc.error_message,
            });
          }
          const res = await api.getResults(targetRunId);
          if (!cancelled && res?.results && res.results.length > 0) {
            usePipelineStore.getState().hydrateRunResults(targetRunId, res.results);
          }
        } catch (e) {
          console.warn('Failed to load run data for', targetRunId, e);
        }
      } else if (activeRunId === 'all' && Object.keys(usePipelineStore.getState().liveTasks).length === 0) {
        // If Global Stream is selected and empty, populate from latest campaign
        try {
          const { runs } = await api.listRuns(1, 0);
          if (!cancelled && runs && runs.length > 0) {
            const latestRun = runs[0];
            const res = await api.getResults(latestRun.id);
            if (!cancelled && res?.results && res.results.length > 0) {
              usePipelineStore.getState().hydrateRunResults(latestRun.id, res.results);
              usePipelineStore.getState().setActiveRunMeta({
                domain: latestRun.domain,
                endpoint_id: latestRun.endpoint_id,
                endpoint_name: latestRun.endpoint_id,
                attacker_model: latestRun.attacker_model,
                judge_model: latestRun.judge_model,
                started_at: latestRun.created_at,
                status: latestRun.status,
                error_message: latestRun.error_message,
              });
            }
          }
        } catch (e) {
          console.warn('Failed to populate latest run for global stream', e);
        }
      }
    };
    void hydrate();
    return () => {
      cancelled = true;
    };
  }, [activeRunId]);

  const liveTasks  = usePipelineStore(s => s.liveTasks);
  const feedCount  = usePipelineStore(s => s.intelligenceFeed.length);
  const tasksArray = useMemo(() => Object.values(liveTasks), [liveTasks]);

  const [viewMode, setViewMode] = useState<'table' | 'grid'>('table');
  // Compact-viewports intel drawer: the fixed right rail is lg+; below that
  // alerts remain reachable instead of silently hidden.
  const [mobileFeedOpen, setMobileFeedOpen] = useState(false);

  const [filters, setFilters] = useState<FilterState>(DEFAULT_FILTERS);

  const handleFilterChange  = useCallback((f: Partial<FilterState>) => setFilters(p => ({ ...p, ...f })), []);
  const handleResetFilters  = useCallback(() => setFilters(DEFAULT_FILTERS), []);

  const filteredTasks = useMemo(
    () => tasksArray.filter(task => matchesFilters(task, filters)),
    [tasksArray, filters]
  );

  const selectedTaskId = selectedEntity?.type === 'task' ? selectedEntity.id : null;
  const handleSelectTask = useCallback((id: string) => openInspector({ type: 'task', id }), [openInspector]);
  const handleOpenDiff = useCallback((taskId: string) => openPromptDiff(activeRunId || 'all', taskId), [openPromptDiff, activeRunId]);
  const hasActiveFilters = filtersActive(filters);

  return (
    // Full-viewport two-pane layout — no scrolling at this level
    <div className="flex flex-col h-full min-h-0 animate-fade-in w-full relative">

      {/* ── Valerie ASCII Specimen Logo Header ── */}
      <div className="w-full flex items-center justify-center py-6 mb-4 select-none overflow-hidden shrink-0">
        <pre
          className="font-mono font-black text-slate whitespace-pre overflow-hidden select-none text-[0.24rem] sm:text-[0.30rem] md:text-[0.36rem]"
          style={{ WebkitTextStroke: '0.4px var(--color-slate)', lineHeight: '1.08' }}
          aria-hidden="true"
        >
          {HERO_ASCII_ART}
        </pre>
      </div>

      {/* ── Unified Command Bar (title + scope + search + launch + filters) ── */}
      <CommandBar
        filters={filters}
        onFilterChange={handleFilterChange}
        onResetFilters={handleResetFilters}
        viewMode={viewMode}
        onViewModeChange={setViewMode}
      />

      {/* ── Critical Execution Failure Banner ── */}
      {activeRunMeta?.error_message && (
        <div className="w-full bg-maroon-muted border-b border-maroon/30 px-4 py-3 select-none animate-fade-in font-mono">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
            <div className="flex items-start gap-2.5 min-w-0">
              <AlertTriangle className="text-maroon shrink-0 mt-0.5" size={16} />
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <span className="font-bold text-xs uppercase tracking-wider text-maroon">
                    CRITICAL EXECUTION FAILURE · SWEEP TERMINATED
                  </span>
                  <span className="text-[10px] text-taupe font-mono">#{activeRunId?.slice(0, 8)}</span>
                </div>
                <div className="text-[11px] text-slate mt-1.5 font-mono break-all bg-ivory border border-maroon/20 p-2.5 rounded-2xs shadow-2xs">
                  <div className="text-[9px] text-maroon font-bold uppercase tracking-wider mb-1">
                    EXECUTION ERROR LOG:
                  </div>
                  <code>{activeRunMeta.error_message}</code>
                </div>
              </div>
            </div>

            <div className="flex items-center gap-2 self-end md:self-center shrink-0">
              <button
                onClick={openLauncher}
                className="flex items-center gap-1.5 px-3 py-1.5 bg-maroon text-parchment font-bold text-[10px] uppercase tracking-wider hover:bg-maroon/90 transition-colors cursor-pointer shadow-xs"
              >
                <RefreshCw size={11} />
                RELAUNCH SWEEP
              </button>
              <button
                onClick={() => { setActiveRun('all'); setActiveRunMeta(null); }}
                className="px-2.5 py-1.5 border border-hairline bg-ivory text-steel hover:text-slate text-[10px] uppercase tracking-wider transition-colors cursor-pointer"
              >
                DISMISS
              </button>
            </div>
          </div>
        </div>
      )}

      {/* ── Instrument Cluster ── */}
      <InstrumentCluster />

      {/* ── Execution Circuit ── */}
      <ExecutionCircuit
        onFilterByStatus={status => handleFilterChange({ status: status as FilterState['status'] })}
        activeStatusFilter={filters.status}
      />

      {/* ── Compact-viewport intel trigger (rail is lg+) ── */}
      {feedCount > 0 && (
        <button
          onClick={() => setMobileFeedOpen(true)}
          className="lg:hidden flex items-center gap-2 px-3 py-2 hairline-bottom bg-camel-muted text-camel font-mono text-[10px] font-bold uppercase tracking-wider cursor-pointer shrink-0"
        >
          <AlertTriangle size={12} />
          {feedCount} INTEL ALERT{feedCount === 1 ? '' : 'S'} — VIEW FEED
        </button>
      )}

      {/* ── Main split-pane: Ledger (left) + Intel Feed (right) ── */}
      <div className="flex flex-1 min-h-0 overflow-hidden">

        {/* Ledger — scrollable independently with stable layout frame */}
        <div className="flex-1 min-w-0 overflow-y-auto" aria-label="Forensic Specimen Ledger">
          <SpecimenLedger
            tasks={filteredTasks}
            selectedTaskId={selectedTaskId}
            onSelectTask={handleSelectTask}
            onOpenDiff={handleOpenDiff}
            viewMode={viewMode}
            hasActiveFilters={hasActiveFilters}
            onResetFilters={handleResetFilters}
          />
        </div>

        {/* Intelligence Feed — fixed-width right rail (lg+) */}
        <div className="hidden lg:flex lg:w-80 shrink-0 flex-col min-h-0">
          <IntelligenceFeed />
        </div>
      </div>

      {/* Intel drawer for compact viewports */}
      {mobileFeedOpen && (
        <div className="fixed inset-0 z-50 lg:hidden" role="dialog" aria-label="Intelligence feed">
          <div
            className="absolute inset-0 bg-slate/40 backdrop-blur-sm"
            onClick={() => setMobileFeedOpen(false)}
          />
          <div className="absolute top-0 right-0 h-full w-[22rem] max-w-[92vw] bg-parchment border-l border-hairline shadow-2xl flex flex-col">
            <div className="flex items-center justify-between px-4 py-3 bg-linen hairline-bottom">
              <span className="font-mono text-[10px] font-bold tracking-widest uppercase text-slate">INTELLIGENCE FEED</span>
              <button onClick={() => setMobileFeedOpen(false)} className="text-steel hover:text-slate cursor-pointer p-1" aria-label="Close intelligence feed">
                <X size={14} />
              </button>
            </div>
            <div className="flex-1 overflow-y-auto min-h-0">
              <IntelligenceFeed />
            </div>
          </div>
        </div>
      )}
    </div>
  );
}
