      ...
      <TelemetryRow
        ...
        cells={[
          ...
          },
          {
            ...
            variant: 'static',
            value: <span className="text-slate">DBSCAN (ε=0.15)</span>,
            sublabel: 'UNSUPERVISED CONVERGENCE',
          },
        ]}
      />
        ...
        <div className="p-4 sm:p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {weaknesses.map((w, idx) => {
            ...
            return (
              <div
                key={w.id || idx}
                className="flex flex-col justify-between bg-ivory border border-hairline hover:border-slate/40 transition-all p-5 space-y-4 shadow-2xs group"
              >
                <div>
                  <div className="flex items-center justify-between text-xs text-steel mb-2">
                    <span className="font-bold tracking-wider text-slate">CLUSTER #{indexStr}</span>
                    <span className="text-[9px] uppercase px-2 py-0.5 bg-linen border border-hairline text-slate font-bold">
                      {(w.trend || 'stable').toUpperCase()} PATTERN
                    </span>
                  </div>
                  <h3 className="text-xs font-bold uppercase tracking-tight text-slate group-hover:text-maroon transition-colors line-clamp-2">
                    {w.name}
                  </h3>
                  <p className="text-xs text-steel font-sans leading-relaxed mt-2 line-clamp-3">
                    {w.description}
                  </p>
                </div>

                <div className="space-y-3 pt-3 hairline-top">
                  <div className="text-[11px] text-steel space-y-1.5 font-mono">
