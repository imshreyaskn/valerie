...
import {
  ...
  ExternalLink, Copy, Check, Filter, RotateCcw, Network, AlertTriangle
} from 'lucide-react';
      ...
      {runsResource.loading && runs.length === 0 ? (
        ...
        viewMode === 'grid' ? (
              ...
              return (
                ...
                >
                  ...
                  <div>
                    ...
                    {run.error_message && (
                      <div className="mt-1.5 flex items-center gap-1 text-[10px] text-maroon font-mono bg-maroon-muted/60 border border-maroon/20 px-2 py-0.5 rounded-2xs" title={run.error_message}>
                        <AlertTriangle size={11} className="shrink-0 text-maroon" />
                  ...
                  <div className="flex items-center justify-between font-mono text-xs pt-2 hairline-top">
                    <div>
                      ...
                      <span className="text-[11px] text-steel">{parseUtcDate(run.created_at).toLocaleDateString()}</span>
                    </div>
                  ...
                  <div className="pt-2 flex items-center justify-between gap-2">
                    <div className="flex items-center gap-1.5">
                      <button
                        ...
                        className="flex items-center gap-1 px-2 py-1 font-mono text-[9px] font-bold uppercase tracking-wider text-steel border border-hairline hover:text-slate hover:border-steel/60 transition-colors cursor-pointer"
                      ...
                      <button
                        onClick={(e) => {
                          ...
                          openLauncher({
                            ...
                            attacker_model: run.attacker_model,
                            judge_model: run.judge_model,
                            max_iterations: (run as any).max_iterations ?? 3,
                            sample_size: (run as any).sample_size ?? 'all',
                            risk_threshold: (run as any).risk_threshold ?? 0.7,
                            techniques: (run as any).techniques || ['indirect_prompting', 'obfuscation', 'role_play'],
                          });
                        }}
                        className="p-1 font-mono text-slate bg-linen/70 border border-hairline hover:bg-slate hover:text-parchment hover:border-slate transition-colors cursor-pointer"
                        title="Clone & re-run campaign configuration"
                        aria-label="Clone campaign configuration"
                      >
                        <RotateCcw size={12} />
                      </button>
                    </div>
          ...
          <div className="w-full hairline-bottom select-none font-mono" role="region" aria-label="Campaigns Table">
            {/* Table Headers */}
            <div className="hidden md:grid md:grid-cols-[50px_160px_1fr_140px_100px_105px_120px] items-center p-0 hairline-bottom bg-linen/30 text-xs font-semibold uppercase text-steel">
              ...
              {filteredRuns.map((run, idx) => {
                ...
                return (
                  <div key={run.id} className="transition-colors hover:bg-linen/40 bg-ivory">
                    <div
                      onClick={() => handleOpenInMissionControl(run)}
                      className="grid grid-cols-1 md:grid-cols-[50px_160px_1fr_140px_100px_105px_120px] items-center p-0 cursor-pointer group"
                      role="button"
                      tabIndex={0}
                    ...
                    >
                      ...
                      <div className="p-3 md:p-4 md:hairline-right min-w-0 text-xs">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-steel text-[11px] truncate">{parseUtcDate(run.created_at).toLocaleString()}</span>
                        </div>
